...
function push() {
    ...
    curl -F document=@$ZIP "https://api.telegram.org/bot$token/sendDocument" \
        ...
        -F "parse_mode=html" \
        -F caption="Build took $(($DIFF / 60)) minute(s) and $(($DIFF % 60)) second(s). | For <b>Mi 10T Pro (apollo)</b> | <b>${KBUILD_COMPILER_STRING}</b>"
}
# Fin Error
function finerr() {
    curl -s -X POST "https://api.telegram.org/bot$token/sendMessage" \
...
function compile() {
    ...
    cp out/arch/arm64/boot/dts/vendor/qcom/kona-v2.1.dtb AnyKernel3/dtb
...
function zipping() {
    ...
    zip -r9 aRise-BETA-kernel-apollo-${TANGGAL}.zip *
    cd ..
}
# Clean
function clean() {
